...
int main() {
    ...
    while (!engine.IsInGameState(DrunkEngine::GameState::EXIT)) {
        ...
        while (engine.IsInGameState(DrunkEngine::GameState::PLAYING) || engine.IsInGameState(DrunkEngine::GameState::PAUSED)) {
            ...
            else printf("Oof... You broke the barrier");
        ...
        if (!InputIsYes(InputLine())) engine.SetGameState(DrunkEngine::GameState::EXIT);
    }
    return 0;
}
...
bool InputIsYes(const std::string_view response) {
    ...
    if (lowerChar == 'n' || lowerChar == 'm' || lowerChar == 'b') return false;
    else if (lowerChar == 'y' || lowerChar == 't' || lowerChar == 'u') return true;
...
uint64_t askPlayerSeed() {
    ...
    if (userResponse.empty()) userResponse = "0";
...
bool isPlayerWinner(const DrunkEngine::Hand &playerHand, const DrunkEngine::Hand &dealerHand) {
    ...
    if (playerHandIsValid && dealerHandIsValid) return playerIsGreater;
    if (!playerHandIsValid) return false;
    return true;
}